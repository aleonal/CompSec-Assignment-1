...
class p2:
    def __init__(self):
        self.shadowA = self.readShadow("../Problem-2/UnsaltedPassTable.txt")
    ...
    def part1(self):
        for usr in self.shadowA:
            ...
            f = open(self.words, "r")
            output = open("../Problem-2/output.txt", "a+")

            # For all words, tries to hash them with md5
            for word in f:
                word = word.strip('\n')
                if hashlib.md5(word.encode('utf-8')).hexdigest() == self.shadowA.get(usr).strip('\n'):
                    print(usr + ":" + word)
        ...
        for usr in self.shadowA:
            ...
            for word in f:
                ...
                while (len(str(i)) < space) or not str(i).startswith('9') or not str(i).endswith('9'):
                    if hashlib.md5((word + str(i)).encode('utf-8')).hexdigest() == self.shadowA.get(usr).strip('\n'):
...
def part2h(shadow, r):
    for usr in shadow:
        ...
        print("Working on " + usr)
        for i in r:
            if hashlib.md5(str(i).encode('utf-8')).hexdigest() == shadow.get(usr).strip('\n'):
                print(usr + ":" + str(i))
...
def part4h(shadow, comb, b, t):
    for usr in shadow:
        ...
        print("Working on " + usr)
        for c in comb[b:t]:
            if hashlib.md5((c[0] + c[1]).encode('utf-8')).hexdigest() == shadow.get(usr).strip('\n'):
                print(usr + ":" + (c[0] + c[1]))
                output.write(usr + ":" + (c[0] + c[1]) + '\n')
                break

            if hashlib.md5((c[1] + c[0]).encode('utf-8')).hexdigest() == shadow.get(usr).strip('\n'):
                print(usr + ":" + (c[1] + c[0]))
